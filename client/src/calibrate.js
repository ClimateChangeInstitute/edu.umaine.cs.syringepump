...
$(document).ready(function() {
  ...
  $("#rootwizard .finish").click(function() {
    var v1 = Number($("#input1kFinal").val());
    var v2 = Number($("#input2kFinal").val());
    var v3 = Number($("#input4kFinal").val());
    ...
    var mTop =
      ...
      (4000 - xMean) * (v3 - yMean);
    var mBottom =
      (1000 - xMean) * (1000 - xMean) +
      (2000 - xMean) * (2000 - xMean) +
      (4000 - xMean) * (4000 - xMean);
    var m = mTop / mBottom; // ml per step
    // We don't use the b value.
    // eslint-disable-next-line no-unused-vars
    var b = yMean - m * xMean;
    ...
    $.getJSON("defaults?action=load", function(data) {
      ...
      var newDiameter =
        2 * Math.sqrt((m * mm3PerMl * stepsPerRev) / (3.14159 * currentPitch));
      ...
      bootbox.confirm(
        "Are you sure you want to save the new calibration settings? " +
          "The new number of steps per 1 ml will be " +
          stepsPerMl +
          ".",
        function(result) {
          if (result) {
    ...
    $("#" + buttonName).click(function(event) {
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          sp.checkLevels(300, 0, min, max, isLoad, "steps");
        },
        error: function(msg) {
          console.log(`Failed to start. Reason: ${msg}`);
        }
      });
    });
  };
